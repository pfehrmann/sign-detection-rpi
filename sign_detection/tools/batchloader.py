...
class BatchLoader(object):
    ...
    def generate_regions_from_image(self, size=64, regions_per_roi=10, regions_per_image=3, overlap=0.75):
        ...
        no_signs = self.generate_windows_without_signs(size=size, regions_per_image=regions_per_image,
                                                       max_overlap=overlap / 2, image=image, raw=raw)
...
def get_images_and_regions(gtsdb_root, min_index=0, max_index=599, shuffle_result=True):
    """
    ...
    return images[min_index:max_index]
